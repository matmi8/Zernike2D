...
mean_list1 <- lapply(
  ...
  \(i) mean(df_surf_bp1$bp[dist_surf1[i, ] <= 6], na.rm = TRUE)
)
rm(dist_surf1)
...
mean_list2 <- lapply(
  ...
  \(i) mean(df_surf_bp2$bp[dist_surf2[i, ] <= 6], na.rm = TRUE)
)
rm(dist_surf2)
...
auc_rocr1 <- get_auc_rocr(unlist(mean_list1), df_surf_bp1$bs)
auc_rocr2 <- get_auc_rocr(unlist(mean_list2), df_surf_bp2$bs)
